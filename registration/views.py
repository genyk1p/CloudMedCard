...
# Регистрация нового пользователя, заполняем форму с данными, формируем инстанс временного юзера, отправляем ссылку на
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():  # Если форма заполнена правильно
            ...
            # Проверочный урл не хотел формироваться по-человечески, так что я его собрал из кусочков
            url = str(request.build_absolute_uri(reverse('index')) + 'accounts/email-confirm-page/' + str(token) + '/')
            s1 = gettext('To confirm registration on https://xyz.com/ please follow the link ')
            ...
            s5 = gettext('Best wishes, administration of xyz.com')
            em = EmailMessage(
                subject=gettext('Registrations on xyz.com '),
                body=s1 + url + ', ' + "\n" + s2 + str(token) + s3 + "\n" + s4 + "\n" + s5,
            ...
            #     return render(request, 'catalog/standard_message_with_button_mci.html', context)
